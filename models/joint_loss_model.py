...
class ColorSeasonClassifier(nn.Module):
    def __init__(self, num_seasons=4, num_subtypes=3):
        ...
        self.vit = vit_b_16(weights=ViT_B_16_Weights.IMAGENET1K_V1) # pre-trained ViT
...
def train(model, train_loader, val_loader, optimizer, criterion, device, epochs, alpha=0.4, beta=0.4, gamma=0.2):
    ...
    for epoch in range(epochs):
        ...
        epoch_train_loss = running_loss / len(train_loader) # average training loss per batch in this epoch
        ...
        # Validation
        val_loss, val_season_acc, val_subtype_acc, val_full_acc = val(model, val_loader, criterion, device, alpha, beta, gamma)
    ...
    return train_loss_arr, season_acc_arr, subtype_acc_arr, full_acc_arr, val_loss_arr, val_season_acc_arr, val_subtype_acc_arr, val_full_acc_arr